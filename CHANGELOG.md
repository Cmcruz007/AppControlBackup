...
\- 📊 Caché de detección de tabla de sesiones de Veeam (reduce queries a INFORMATION\_SCHEMA)

\- 🌍 Soporte de proxy corporativo mediante `undici` (HTTP\_PROXY / HTTPS\_PROXY)
...
\- 🐛 Las modificaciones de criticidad no se guardaban (`(window as any).api` duplicado en los paneles)
...
\- 🐛 El modal se cerraba al arrastrar el ratón desde dentro hacia fuera (selección de texto)
...
\- ⚠️ La API REST aún no tiene autenticación por token (`BM\_AUTH\_TOKEN`)
