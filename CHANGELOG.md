...
#### 🚀 AÑADIDO
- Modal de log del Histórico (vista por calendario): los títulos ya no llevan el prefijo fijo "LOG AS/400 -". Ahora se muestran con el nombre real del job (por ejemplo, Backup VDC Sharepoint y Teams, Backup Barracuda Exchange, Backup AS400 RR).
- Modal de log del Histórico: colores restaurados por tipo de job AS400 (SD verde, PR rojo, RR amarillo, SDB/TGT azul), en vez de mostrar siempre el mismo verde fijo.

#### 🐛 CORREGIDO
- **Histórico por calendario sin contenido de log en VDC y Barracuda**: `evaluateVdcRule` y `evaluateBarracudaRule` (electron/modules/rules.cjs) descargaban y parseaban el cuerpo del correo, pero nunca lo adjuntaban a la fila devuelta al frontend. El modal de log abría correctamente pero mostraba siempre "No hay contenido o no se pudo extraer.". Corregido añadiendo el campo `logContent` a la fila en ambas funciones, igual que ya ocurría con `as400LogContent` en AS400.
- HistoryTab.tsx: el modal de log solo leía el campo `as400LogContent`, por lo que únicamente los jobs AS400 podían mostrar contenido aunque el resto de fuentes tuvieran datos disponibles. Corregido para leer el contenido con una cadena de prioridad (`logContent`, `as400LogContent`, `logText`, `emailLog`, `bodyContent`, `body`, `bodyPreview`).

#### 🔧 INTERNO
- electron/modules/graph.cjs (`getJobExecutionsFromEmailHistory`): para VDC y Barracuda, si Microsoft Graph no devuelve el cuerpo completo de un correo histórico, se usa `bodyPreview` como respaldo tanto para mostrar el log como para intentar extraer fecha y estado. Refuerza la resiliencia del histórico de ejecuciones por job (pantalla distinta del Histórico por calendario), sin alterar la lógica de estado, duración ni horarios ya validada.
- electron/modules/rules.cjs (`buildAs400Rows`): se añade también el campo genérico `logContent` junto a `as400LogContent`, por consistencia con VDC y Barracuda.

#### ✅ VALIDADO
- Backup AS400 RR: título, color amarillo y contenido correctos en el Histórico por calendario (ya funcionaba antes de esta versión).
- Backup VDC Sharepoint y Teams: título correcto y contenido del log recuperado en el Histórico por calendario.
- Backup Barracuda Exchange: título correcto y contenido del log recuperado en el Histórico por calendario.
- Cambios desplegados y verificados en producción (DASHBOARD) tras `npm run build` y reinicio del servicio BackupMonitor.
