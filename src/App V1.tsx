...
export default function App() {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 800px; padding: 20px;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <thead style="background: #f1f5f9;">
            <tr><th style="padding: 10px; text-align: left;">Estado</th><th style="padding: 10px; text-align: left;">Job</th><th style="padding: 10px; text-align: left;">Hora</th><th style="padding: 10px; text-align: left;">Duración</th></tr>
          </thead>
  ...
  async function handleSendEmail() {
    ...
    setIsSending(true);
    const res = await window.api.sendEmail({ to: emailTo, subject: `MONITOR BACKUPS: ${new Date().toLocaleDateString()}`, bodyHtml: emailPreviewHtml });
    setIsSending(false);
  ...
  return (
    <div className="app compact-mode">
      <div className="topbar">
        ...
        <div className="meta">{lastRun ? `Actualizado: ${new Date(lastRun).toLocaleTimeString()}` : "Cargando..."}</div>
      </div>
          ...
          <>
            ...
            <div className="toolbar">
              ...
              <button onClick={() => setShowAll(!showAll)} className="secondary">
                {showAll ? "Ver solo Errores" : "Mostrar todos"}
              </button>
            ...
            <table className="compact-table">
              <thead>
                <tr>
                  ...
                  <th onClick={() => toggleSort("email")} style={{textAlign:'center'}}>Email</th>
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr><td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: '#94a3b8' }}>¡Todo correcto! No hay incidencias pendientes.</td></tr>
                )}
                {filtered.map(r => (
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="detail-grid" style={{ marginTop: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <div><strong>ID Sesión:</strong> {detail.jobId}</div>
                <div><strong>Programado:</strong> {formatLocal(detail.nextRun)}</div>
                <div><strong>Duración Total:</strong> {formatDuration(detail.durationMs)}</div>
                <div><strong>Email recibido:</strong> {detail.email ? "SÍ" : "NO"}</div>
              </div>
              {detail.allEmails && detail.allEmails.length > 0 && (
                <div className="email-log" style={{ marginTop: '20px' }}>
                  <h3>Historial de correos en esta ventana:</h3>
      ...
      {emailModal && (
        <div className="modal-bg">
          <div className="modal" style={{ maxWidth: '800px' }}>
            <div className="modal-header">
              <h2>📧 Enviar Reporte Diario</h2>
              <button className="close-btn" onClick={() => setEmailModal(false)}>×</button>
            </div>
            <div className="form-group" style={{ marginBottom: '15px' }}>
              <label>Destinatario:</label>
              <input style={{ width: '100%', padding: '8px', marginTop: '5px' }} type="email" value={emailTo} onChange={e => setEmailTo(e.target.value)} placeholder="ejemplo@correo.com" />
            </div>
