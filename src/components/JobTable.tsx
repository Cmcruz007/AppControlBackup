            ...
            <tr key={r.jobId} className={rowClass}>
              <td>
                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                  <span title={`Criticidad: ${r.criticality ?? "low"}`}
                    style={{
                      width: 10, height: 10, borderRadius: 2, flex: "0 0 auto",
                      background: r.criticality === "high" ? "#ef4444" : r.criticality === "medium" ? "#f59e0b" : "#22c55e",
                      boxShadow: "0 0 0 1px rgba(255,255,255,.08) inset",
                    }} />
                  <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.jobName}</span>
                  {r.relaunched && (
                    <span title="Relanzado" style={{
                      ...
                      color: "#93c5fd", fontWeight: 700, textTransform: "uppercase", flex: "0 0 auto",
                    }}>REL</span>
                  )}
                  {(r as any).as400LogContent && (
                    <button title="Ver log AS/400" style={{
                      background: "none", border: "none", padding: "0 2px", cursor: "pointer",
                      fontSize: 15, lineHeight: 1, flex: "0 0 auto", color: "#34d399",
                      filter: "drop-shadow(0 0 3px rgba(52,211,153,.5))",
                    }} onClick={() => onOpenLog?.(r.jobName)}>📋</button>
                  )}
                </div>
              </td>
              <td>
                <span className={`badge ${displayStatus}`}>
                  {displayStatus === "success" ? "SUCCESS" : displayStatus === "warning" ? "WARNING"
                    : displayStatus === "failed" ? "ERROR" : displayStatus === "running" ? "RUNNING"
                    : displayStatus === "pending" ? "PENDING" : String(displayStatus).toUpperCase()}
                </span>
              </td>
              <td style={{ textAlign: "center" }}><SourceIcon source={r.source} /></td>
              ...
              <td className="tabular">
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {r.durationTrend === "up" && <span title="Tardó >20% más que el anterior" style={{ color: "#ef4444", fontSize: 16, cursor: "help" }}>▲</span>}
                  {r.durationTrend === "down" && <span title="Tardó >20% menos que el anterior" style={{ color: "#22c55e", fontSize: 16, cursor: "help" }}>▼</span>}
                  {r.durationTrend === "same" && <span title="Duración estable (<20%)" style={{ color: "#f59e0b", fontSize: 18, fontWeight: "bold", cursor: "help" }}>=</span>}
                </div>
              </td>
              <td>{displayReason}</td>
              {!readOnly && (
                <td style={{ whiteSpace: "nowrap" }}>
                  <button className="secondary" style={{ padding: "4px 8px", fontSize: 12, marginRight: 6 }}
                    onClick={() => onOpenExecutions?.(r.jobName)}>Backups</button>
                  <button className="secondary" style={{ padding: "4px 8px", fontSize: 12 }}
                    onClick={() => onEditComment?.(r.jobId)}>Editar</button>
                </td>
              )}
            </tr>
          )
