...
function makeId() {
  return `vdc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
}

function normalizeRule(rule: any, index: number): VeeamDataCloudRule {
  return {
    id: String(rule?.id || `vdc-rule-${index + 1}` || makeId()),
...
export default function VeeamDataCloudPanel({
  config,
  onSaved,
}: {
  ...
  const initialRules = useMemo(
    () => normalizeRules((config as any)?.veeamDataCloudRules ?? []),
    [config],
  )
  ...
  useEffect(() => {
    setRules(normalizeRules((config as any)?.veeamDataCloudRules ?? []))
  }, [config])

  // Usamos (prev) para evitar el problema de Stale Closures y el "Force Reload"
  function addRule() {
    setRules((prev) => [
      ...prev,
      {
  ...
  async function saveAll() {
    if (!config) return
    const nextCfg = { ...config, veeamDataCloudRules: rules }
    
    // Inyección de la configuración completa al backend
    const ok = await api().saveConfig(nextCfg)
    if (ok) {
      onSaved(nextCfg)
      alert('Veeam Data Cloud guardado correctamente.')
    } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0, color: 'var(--text)' }}>Reglas de Veeam Data Cloud</h3>
      ...
      {rules.length === 0 ? (
        <div style={{ color: 'var(--muted)', fontStyle: 'italic', textAlign: 'center', padding: 20 }}>
          No hay reglas configuradas para Veeam Data Cloud.
        </div>
      ) : (
