  ...
  const tableRows = emailRows.map((r, i) => {
    ...
    return `
      <tr bgcolor="#${bg}">
        ...
        <td width="100" style="padding:10px 12px;border-top:1px solid #1e3a5f;font-size:12px;color:#e2e8f0;font-family:'Courier New',monospace;text-align:center;white-space:nowrap;min-width:100px">${escapeHtml(formatDuration(r.durationMs))}</td>
        <td width="350" style="padding:10px 12px;border-top:1px solid #1e3a5f;font-size:12px;color:#cbd5e1;font-family:Arial,sans-serif;white-space:nowrap;min-width:350px">${escapeHtml(r.reason)}</td>
      </tr>`
  }).join("")
