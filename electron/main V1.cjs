...
function setupIpc() {
  ...
  ipcMain.handle("refresh", async () => {
    ...
    try {
      ...
      // 1. DEDUPLICACIÓN AGRESIVA POR CONTENIDO
      ...
      // 2. PREVALENCIA DE SUCCESS (Monitor de Excepciones)
      ...
      const allRows = uniqueExecs.map(s => {
        ...
        let prog = null, match = s.job_name.match(/(\d{2})(\d{2})/);
        if (match) prog = new Date(start.getFullYear(), start.getMonth(), start.getDate(), parseInt(match[1]), parseInt(match[2]), 0);
        else prog = new Date(start.getFullYear(), start.getMonth(), start.getDate(), start.getHours(), 0, 0);
        ...
        return {
          ...
          status,
          reason,
          durationMs: (end - prog) > 0 ? (end - prog) : null,
          email: email ? { subject: email.subject, date: email.receivedDateTime } : null,
          allEmails: relevantEmails.map(e => ({ subject: e.subject, date: e.receivedDateTime, status: e.bodyPreview?.toLowerCase().includes("success") ? "success" : "failed" }))
        };
      });

      // Filtro para vista inicial (Solo excepciones que NO han tenido éxito hoy)
